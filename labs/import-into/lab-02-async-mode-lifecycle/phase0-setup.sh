...
mysql -h "${TIDB_HOST}" -P "${TIDB_PORT}" -u "${TIDB_USER}" \
  ...
  -e "SELECT VERSION(), @@tidb_version\\G" \
...
TIDB_VERSION_LINE="$(grep -i '@@tidb_version' -A1 "${RESULTS_DIR}/tidb-version.txt" | tail -1 || true)"
echo "[phase0] tidb version probe: ${TIDB_VERSION_LINE}"
...
# ---------- 0.5: persist env for subsequent phases ----------

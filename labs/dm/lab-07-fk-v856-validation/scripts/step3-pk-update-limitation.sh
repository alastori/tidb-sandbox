...

{
    ...
    echo "  - child_cascade for parent_id=3: ORPHANED (FK_CHECKS=0 bypasses CASCADE on target)"
    echo "  - child_restrict for parent_id=3: ORPHANED (FK_CHECKS=0 bypasses RESTRICT)"
    echo "  - child_setnull for parent_id=3: ORPHANED (FK_CHECKS=0 bypasses SET NULL)"
    echo "  - All children retain parent_id=3 but the parent row no longer exists"
